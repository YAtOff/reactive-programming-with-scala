...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe { t =>
      t match {
    ...
    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.flatMap(b => Observable.from(suggestionList.selection.items))

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.concatRecovered(wikiPageResponseStream _)

    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe { t =>
      t match {
