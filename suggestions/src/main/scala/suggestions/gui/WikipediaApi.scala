...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      val endTime = System.currentTimeMillis() + totalSec * 1000
      obs.timestamp.timeout(
      { () => Observable.timer(Duration(totalSec, "second"))
      },
      {
        case (ts, v) => Observable.timer(Duration(endTime - ts, "millis"))
      },
      Observable.empty
      ).map {
        case (a, b) => b
